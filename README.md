# ☕ CafeKiosk

> **Practical Testing: 실용적인 테스트 가이드** (박우빈) 강의를 수강하며 진행하는 학습 프로젝트입니다.
> 카페 키오스크라는 작은 도메인을 소재로, **"어떤 테스트를 어떻게 작성해야 소프트웨어의 안정성과 신뢰성을 보장할 수 있는가"** 를 처음부터 다시 배웁니다.

<br/>

## 📌 이 프로젝트를 시작한 이유

팀 프로젝트를 진행하면서 테스트 코드를 작성해야 한다는 것은 알고 있었지만, **무엇을 어떻게 테스트해야 하는지는 몰랐습니다.**
결국 AI에게 맡겨 테스트 코드를 채워 넣었고, 커버리지 숫자는 올라갔지만 남은 건 찜찜함이었습니다.

- 테스트가 통과해도 이 코드가 정말 동작한다는 확신이 들지 않았습니다.
- 프로덕션 코드를 조금만 고쳐도 테스트가 우수수 깨졌고, 그때마다 테스트를 "통과하도록" 수정하고 있었습니다.
- 무엇을 검증하려는 테스트인지, 나중에 읽는 사람이 이해할 수 없는 테스트가 쌓여갔습니다.
- 결국 이런 의문이 남았습니다. **"이 테스트 코드, 실용적인가?"**

테스트 코드는 작성하는 것 자체가 목적이 아니라 **변경에 대한 두려움을 없애고, 코드의 의도를 문서로 남기고, 리팩토링을 가능하게 만드는 수단**이어야 한다고 생각했습니다.
그 기준을 스스로 세우기 위해 이 강의를 수강하게 되었습니다.

🔗 **강의 링크** — [Practical Testing: 실용적인 테스트 가이드](https://www.inflearn.com/course/practical-testing-%EC%8B%A4%EC%9A%A9%EC%A0%81%EC%9D%B8-%ED%85%8C%EC%8A%A4%ED%8A%B8-%EA%B0%80%EC%9D%B4%EB%93%9C?cid=329295)

<br/>

## 🛠 기술 스택

| 구분 | 내용 |
| --- | --- |
| Language | Java 17 |
| Framework | Spring Boot 2.7.7 |
| Build | Gradle (Groovy DSL) · Gradle Wrapper 7.6.4 |
| Persistence | Spring Data JPA · H2 Database |
| Test | JUnit 5 · AssertJ · Mockito · Spring Boot Test |
| Etc | Lombok |

<br/>

## 🚀 실행 방법

```bash
# 클론
git clone https://github.com/woo-in/cafekiosk.git
cd cafekiosk

# 테스트 실행
./gradlew test

# 애플리케이션 실행
./gradlew bootRun
```

- H2 콘솔: <http://localhost:8080/h2-console> (JDBC URL: `jdbc:h2:mem:~/cafekiosk`, User: `sa`)

<br/>

## 📚 이 강의와 프로젝트를 통해 배운 것

> 강의를 따라가며 계속 채워 나가는 섹션입니다. 학습이 끝난 항목은 체크되어 있습니다.

### 1. 테스트는 왜 필요한가 — 자동화된 테스트의 가치

- [ ] 커버할 수 없는 영역이 계속 남는 **수동 테스트의 한계**와, 그로 인해 쌓이는 유지보수 비용
- [ ] 자동화된 테스트가 주는 것: **빠른 피드백**, 변경에 대한 자신감, 그리고 **리팩토링을 가능하게 만드는 안전망**
- [ ] 테스트를 작성하는 비용보다, 테스트가 없어서 치르는 비용이 훨씬 크다는 관점

### 2. 단위 테스트 (Unit Test)

- [ ] **JUnit 5**와 **AssertJ**로 작은 코드 단위를 빠르고 독립적으로 검증하기
- [ ] 테스트 케이스 세분화 — **해피 케이스 / 예외 케이스**, 그리고 반드시 짚어야 할 **경계값 테스트**
- [ ] 요구사항에서 테스트할 것과 테스트하지 않을 것을 구분하는 기준 세우기
- [ ] **테스트하기 어려운 영역을 분리하기** — 현재 시각, 랜덤 값, 외부 API 같은 제어 불가능한 요소를 인자로 밀어내어 순수 함수로 만들기

### 3. TDD (Test Driven Development)

- [ ] **RED → GREEN → REFACTOR** 사이클을 몸으로 익히기
- [ ] "테스트를 먼저 쓴다"가 아니라 **"구현부에 대한 관점이 바뀐다"** 는 것의 의미
- [ ] 선(先)기능 구현 vs 선(先)테스트 작성의 차이 — 복잡도가 낮은 설계, 빠른 피드백, 과감한 리팩토링
- [ ] 클라이언트 관점에서 API를 먼저 설계하게 되는 효과

### 4. 테스트는 [ ]다 — 문서로서의 테스트

- [ ] 테스트는 **살아있는 문서**다: 프로덕션 코드의 요구사항과 히스토리를 담는다
- [ ] **`@DisplayName`을 섬세하게** — "~테스트"가 아니라, 도메인 용어로 **행위와 결과**를 문장으로 서술하기
- [ ] **BDD 스타일** 작성법 — `Given` / `When` / `Then` 으로 시나리오 드러내기
- [ ] 팀의 지식이 코드에 남아 다음 사람에게 전달되는 구조 만들기

### 5. Spring & JPA 기반 테스트 — 레이어별 전략

- [ ] **Layered Architecture**와 통합 테스트의 필요성
- [ ] **Persistence Layer** — `@DataJpaTest`로 데이터 접근 로직만 빠르게 검증하기
- [ ] **Business Layer** — `@SpringBootTest`로 비즈니스 로직 + 트랜잭션 경계 검증하기
- [ ] **Presentation Layer** — `@WebMvcTest` + `MockMvc` + `@MockBean`으로 요청/응답과 **유효성 검증** 테스트하기
- [ ] `@Transactional(readOnly = true)`의 의미와 **CQRS** 관점의 읽기/쓰기 분리
- [ ] 통합 테스트와 단위 테스트의 경계를 어디에 둘 것인가

### 6. Mock을 마주하는 자세

- [ ] **Test Double**의 종류 — Dummy, Fake, Stub, Spy, Mock의 차이와 목적
- [ ] `@Mock`, `@Spy`, `@InjectMocks`, `@MockBean`을 상황에 맞게 골라 쓰기
- [ ] **BDDMockito**로 `given().willReturn()` 형태의 읽기 좋은 스텁 작성하기
- [ ] **Classicist vs Mockist** — 언제 진짜 객체를 쓰고, 언제 대역을 세울 것인가에 대한 나만의 기준

### 7. 더 나은 테스트를 작성하기 위한 구체적 조언

- [ ] **한 문단에 한 주제** — 하나의 테스트는 하나의 논리적 의미만 검증한다 (분기·반복이 들어가는 순간 신호)
- [ ] **완벽하게 제어하기** — 시간·랜덤 등을 파라미터로 주입해 테스트를 결정적으로 만들기
- [ ] **테스트 환경의 독립성 / 테스트 간 독립성** — 공유 자원 제거, `deleteAll()`과 `deleteAllInBatch()`의 차이
- [ ] **한 눈에 들어오는 Test Fixture** — `@BeforeEach`에 모든 것을 몰아넣지 않기, 빌더로 의미 있는 픽스처 만들기
- [ ] **`@ParameterizedTest`와 `@DynamicTest`** 로 케이스를 확장하고 시나리오를 표현하기
- [ ] **테스트 수행도 비용이다** — `@SpringBootTest` 남용 피하기, 테스트 환경 통합으로 컨텍스트 재사용하기
- [ ] private 메서드는 테스트하지 않는다 / 테스트만을 위한 코드는 어디까지 허용할 것인가

### 8. Appendix

- [ ] **학습 테스트** — 낯선 라이브러리를 테스트로 학습하고, 그 결과를 팀 자산으로 남기기
- [ ] **Spring REST Docs** — 테스트가 통과해야만 생성되는, 신뢰할 수 있는 API 문서 만들기 (vs Swagger)

<br/>

## 🍩 도메인: 카페 키오스크

강의를 따라가며 아래 요구사항을 하나씩 테스트와 함께 구현합니다.

- 주문 목록에 음료 추가 / 삭제, 전체 삭제
- 주문 목록 총 금액 계산
- 주문 생성 (영업 시간 검증 포함)
- 상품 판매 상태 관리 (판매중 / 판매보류 / 판매중지)
- 재고 차감 및 재고 부족 예외 처리
- 일별 매출 통계

<br/>

## 🗂 프로젝트 구조

```
src
├── main
│   ├── java/sample/cafekiosk
│   │   └── CafekioskApplication.java
│   └── resources
│       └── application.yml
└── test
    └── java/sample/cafekiosk
```

<br/>

## ✍️ 회고

> 강의를 마친 뒤, "AI가 대신 써준 테스트"와 "내가 기준을 가지고 쓴 테스트"가 무엇이 다른지 이 자리에 정리할 예정입니다.

<br/>

---

📖 **원 강의** · [Practical Testing: 실용적인 테스트 가이드 — 박우빈](https://www.inflearn.com/course/practical-testing-%EC%8B%A4%EC%9A%A9%EC%A0%81%EC%9D%B8-%ED%85%8C%EC%8A%A4%ED%8A%B8-%EA%B0%80%EC%9D%B4%EB%93%9C?cid=329295)
